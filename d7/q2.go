...
func three(state *programState, input []int) {
	...
	if state.sysIDptr+1 < len(state.sysID) {
		state.sysIDptr++
	}
...
func runSequences(opCodes map[string]interface{}, input []int, seq1, seq2 [][]int, state programState) int {
...
func main() {
	...
	sequences2 := getSequences([]int{5, 6, 7, 8, 9})
